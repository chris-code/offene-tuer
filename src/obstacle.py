...
class Obstacle:
	def __init__(self, field, x, y, width, height):
		...
		self.rect_id = self.canvas.create_rectangle(self.x - self.half_width, self.y - self.half_height/2, self.x + self.half_width, self.y + self.half_height, fill=obstacle_color)
